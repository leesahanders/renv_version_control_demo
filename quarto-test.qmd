...
Set an environment variable in the code: 

```python
import os
os.environ["TEST"] = "This test was successful"
```

Access Environment Variable later: 
...
You can also wrap it in a try loop if it is a variable you aren't sure is set: 
...
- [Environment variables](https://datagy.io/python-environment-variables/)
...
Create a variable, for example `py_var = 1`, then `quit`

#### System level environment variables 

Environment variables can also be set at the system level (linux, mac, windows, etc). Those system level environment variables can then be pulled into your active Python or R sessions. 

There is a concept of two different levels of variables at the OS level: 

- Shell variable - these variables are contained only inside the current shell. 
- Environment variable - Variables exist in current shell but are inherited by child shells or processes

Try these commands from terminal to see how variables work: 

```bash
#Create a shell variable: 
TEST_VAR='Hello World!'

#Verify it's a shell variable: 
set | grep TEST_VAR

#Verify it is not an environment variable: 
printenv | grep TEST_VAR

#Turn it into an enviornment variable with export: 
export TEST_VAR

#Set an environment variable in a single step, without making it a shell variable first: 
export NEW_VAR="Testing export"

#Check that the new variable is available: 
echo $NEW_VAR

#Turn the variable back into a shell variable: 
export -n TEST_VAR
...
Unfortunately this method doesn't have a way of passing enviornment variables to the parent shell. This is intentional since it prevents programs from impacting operating environments out of scope. 

Notes: 

- `set` is used in windows, `export` for mac or linux. 
- `%%` is used in windows, `$` in linux


These aren't working: 

Read that variable from R: 
...
Read that variable from Python: 
...
- System commands in R scripts: <https://www.r-bloggers.com/2021/09/how-to-use-system-commands-in-your-r-script-or-package/> 
- How to pass bash variable into R script: <https://stackoverflow.com/questions/56777529/how-to-pass-bash-variable-into-r-script> 
- R command line academy: <https://bash-intro.rsquaredacademy.com/r-command-line.html> 
- Read and set environment and shell variables in linux: <https://www.digitalocean.com/community/tutorials/how-to-read-and-set-environmental-and-shell-variables-on-linux> 

#### Add environment variables through the Connect UI
